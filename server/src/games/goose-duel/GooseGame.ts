import { GooseCard, GooseGameState, GoosePlayer, BoardSpace, PlacedTrap, PlayerToken, GOOSE_CARDS, generateBoard } from '../../../../shared/types/goose-duel';

export class GooseGame {
    private gameId: string;
    private players: Map<string, GoosePlayer>;
    private board: BoardSpace[];
    private placedTraps: PlacedTrap[];
    private deck: GooseCard[];
    private discardPile: GooseCard[];
    private currentPlayerIndex: number;
    private turnNumber: number;
    private phase: 'draw' | 'action' | 'movement' | 'event' | 'finished';
    private boardLength: number;
    private lastAction: string;

    constructor(gameId: string, boardLength: number = 50) {
        this.gameId = gameId;
        this.players = new Map();
        this.boardLength = boardLength;
        this.board = generateBoard(boardLength);
        this.placedTraps = [];
    ...
    private createDeck(): GooseCard[] {
        const deck: GooseCard[] = [];
        let cardId = 0;
        ...
        for (let i = 0; i < 40; i++) {
            ...
            deck.push({ id: `card-${cardId++}`, ...GOOSE_CARDS[cardKey] });
        }
    ...
    private drawCard(): GooseCard | null {
    ...
    playMovementCard(playerId: string, cardId: string): { success: boolean; message: string } {
    ...
    placeTrap(playerId: string, cardId: string, position: number): { success: boolean; message: string } {
        const player = this.players.get(playerId);
        if (!player || !player.isCurrentPlayer) {
            return { success: false, message: 'Not your turn!' };
        }
    ...
    useBoost(playerId: string, cardId: string, targetPosition?: number): { success: boolean; message: string } {
        const player = this.players.get(playerId);
        if (!player || !player.isCurrentPlayer) {
            return { success: false, message: 'Not your turn!' };
        }
    ...
    usePowerUp(playerId: string, cardId: string, targetPlayerId?: string): { success: boolean; message: string } {
        const player = this.players.get(playerId);
        if (!player || !player.isCurrentPlayer) {
            return { success: false, message: 'Not your turn!' };
        }
    ...
    private triggerSpaceEffect(player: GoosePlayer): void {
        const space = this.board[player.token.position];
        if (!space || !space.effect) return;
        ...
        switch (space.effect.type) {
            ...
            case 'draw':
                for (let i = 0; i < (space.effect.value || 0); i++) {
                    const card = this.drawCard();
                    if (card && player.hand.length < 5) player.hand.push(card);
                }
    ...
    private checkTraps(player: GoosePlayer): void {
        if (player.hasShield) {
        ...
        const trapIndex = this.placedTraps.findIndex(
            t => t.position === player.token.position && t.playerId !== player.id
        );
    ...
    private findNextGooseSpace(currentPosition: number): BoardSpace | null {
        return this.board.find(space => space.position > currentPosition && space.type === 'goose') || null;
    }

    private getOpponent(playerId: string): GoosePlayer | null {
    ...
    private getCurrentPlayer(): GoosePlayer | null {
        return this.getPlayerByIndex(this.currentPlayerIndex);
    }

    private getPlayerByIndex(index: number): GoosePlayer | null {
        let i = 0;
        for (const player of this.players.values()) {
